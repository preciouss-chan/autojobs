...
For local Ollama, leave the extension API key empty and configure `LLM_PROVIDER=local` or `LLM_BASE_URL` on the Next.js server. For OpenAI or another hosted OpenAI-compatible endpoint, open the extension popup, click **Open dashboard**, and save the key from the dashboard key page.

For Chrome development installs, use `extension/build.sh`, then load the unpacked `extension/build/chrome` folder from `chrome://extensions`. The zip is useful for distribution, but the developer-mode install flow expects an unpacked folder.
