...
async function handleSaveApiKeyFromDashboard(msg) {
  try {
    ...
    await browserAPI.storage.sync.set({ [STORAGE_KEYS.OPENAI_API_KEY]: apiKey });
...
async function handleClearApiKeyFromDashboard() {
  try {
    await browserAPI.storage.sync.remove([STORAGE_KEYS.OPENAI_API_KEY]);
...
async function handleGetApiKeyForDashboard() {
  try {
...
async function handleSaveCoverLetterFromPage(msg) {
  try {
...
async function handleClearCoverLetterFromPage() {
  try {
    await browserAPI.storage.local.remove(["lastCoverLetterBase64", "lastCoverLetterFilename"]);
...
async function handleGetUploadedResumeForDashboard() {
  try {
    const res = await browserAPI.storage.local.get(["uploadedResume", "uploadedResumeFilename"]);
    return {
      ok: true,
...
async function handleSaveTailoredResumePdfFromPage(msg) {
  try {
    const base64 = typeof msg.base64 === "string" ? msg.base64 : "";
    if (!base64) {
      return { ok: false, error: "No resume PDF provided" };
    }
...
browserAPI.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.action === "SAVE_API_KEY_FROM_DASHBOARD") {
    return handleSaveApiKeyFromDashboard(msg);
  }

  if (msg.action === "CLEAR_API_KEY_FROM_DASHBOARD") {
    return handleClearApiKeyFromDashboard();
  }

  if (msg.action === "GET_API_KEY_FOR_DASHBOARD") {
    return handleGetApiKeyForDashboard();
  }

  if (msg.action === "SAVE_COVER_LETTER_FROM_PAGE") {
    return handleSaveCoverLetterFromPage(msg);
  }

  if (msg.action === "CLEAR_COVER_LETTER_FROM_PAGE") {
    return handleClearCoverLetterFromPage();
  }

  if (msg.action === "GET_UPLOADED_RESUME_FOR_DASHBOARD") {
    return handleGetUploadedResumeForDashboard();
  }

  if (msg.action === "SAVE_TAILORED_RESUME_PDF_FROM_PAGE") {
    return handleSaveTailoredResumePdfFromPage(msg);
  }
