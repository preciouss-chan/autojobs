...
window.addEventListener("message", (event) => {
  if (event.source !== window || !isDashboardOrigin(event.origin)) {
    return;
  }

  const data = event.data;
  if (!data || data.source !== "autojobs-dashboard") {
    return;
  }

  if (data.action === "AUTOJOBS_SAVE_API_KEY" && typeof data.apiKey === "string") {
    sendRuntimeMessage({
      action: "SAVE_API_KEY_FROM_DASHBOARD",
      apiKey: data.apiKey
    });
  }

  if (data.action === "AUTOJOBS_CLEAR_API_KEY") {
    sendRuntimeMessage({ action: "CLEAR_API_KEY_FROM_DASHBOARD" });
  }

  if (data.action === "AUTOJOBS_GET_API_KEY") {
    sendRuntimeMessage({ action: "GET_API_KEY_FOR_DASHBOARD" }).then((resp) => {
      window.postMessage(
        {
          source: "autojobs-extension",
          action: "AUTOJOBS_API_KEY_STATUS",
          hasApiKey: Boolean(resp?.apiKey),
          apiKey: resp?.apiKey || ""
        },
        event.origin
      );
  ...
  if (data.action === "AUTOJOBS_SAVE_COVER_LETTER" && typeof data.coverLetter === "string") {
    sendRuntimeMessage({
      action: "SAVE_COVER_LETTER_FROM_PAGE",
      coverLetter: data.coverLetter
    });
  }

  if (data.action === "AUTOJOBS_CLEAR_COVER_LETTER") {
    sendRuntimeMessage({ action: "CLEAR_COVER_LETTER_FROM_PAGE" });
  }

  if (data.action === "AUTOJOBS_GET_UPLOADED_RESUME") {
    sendRuntimeMessage({ action: "GET_UPLOADED_RESUME_FOR_DASHBOARD" }).then((resp) => {
      window.postMessage(
        {
          source: "autojobs-extension",
          action: "AUTOJOBS_RESUME_STATUS",
          hasResume: Boolean(resp?.resume),
          resume: resp?.resume || null,
  ...
  if (data.action === "AUTOJOBS_SAVE_UPLOADED_RESUME" && data.resume) {
    sendRuntimeMessage({
  ...
  if (data.action === "AUTOJOBS_SAVE_TAILORED_RESUME_PDF" && typeof data.base64 === "string") {
    sendRuntimeMessage({
      action: "SAVE_TAILORED_RESUME_PDF_FROM_PAGE",
      base64: data.base64,
      filename: typeof data.filename === "string" ? data.filename : "Tailored_Resume.pdf"
    });
  }
});
