...
async function updateApiKeyBadge() {
  try {
    ...
    console.log("🔄 Updating key badge... API key exists:", !!apiKey);
    const apiKeyState = document.getElementById("apiKeyState");

    if (!apiKeyState) {
      console.warn("API key badge not found");
      return;
    }

    apiKeyState.textContent = apiKey ? `Dashboard · ${maskApiKey(apiKey)}` : "Dashboard key";
  } catch (err) {
    console.error("Error updating key badge:", err);
  }
}

// Initialize popup UI on open
(async () => {
  try {
    ...
    await updateApiKeyBadge();
  } catch (err) {
    console.error("Error initializing popup UI:", err);
  }
})();
...
document.getElementById("dashboardBtn").addEventListener("click", async () => {
  ...
  const apiKey = await getStoredApiKey();
  const dashboardPath = apiKey ? "/dashboard" : "/dashboard/api-key";
  browserAPI.tabs.create({ url: `${BACKEND_URL}${dashboardPath}` });
});
